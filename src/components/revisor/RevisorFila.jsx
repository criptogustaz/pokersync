...
import LeaksCard from "./LeaksCard.jsx";
...
export default function RevisorFila({ onNova, onOpen }) {
  ...
  useEffect(() => {
    (async () => {
      ...
      console.log("DEBUG getCurrentUser:", JSON.stringify(user, null, 2));
  ...
  return (
    <div>
      <div style={{ display: "flex", justifyContent: "flex-end", marginBottom: 12 }}>
        <button style={S.btnPrimary} onClick={onNova}>
          <Plus size={16} />
          Nova mão
        </button>
      </div>
      ...
      {loading ? (
        ...
        <div style={S.empty}>
          ...
          <button style={{ ...S.btnPrimary, marginTop: 8 }} onClick={onNova}>
            <Plus size={16} />
        ...
        <ul style={S.list}>
          {items.map((r) => {
            ...
            return (
              <li key={r.id} style={S.card} onClick={() => onOpen(r.id)}>
